...
int node1( const char *url )
{
	...
	assert( bytes >= 0 );
	printf( "NODE1: RECEIVED DATE %s %d\n", buf, bytes );
	nn_freemsg( buf );
	return nn_shutdown( sock, 0 );
}

// usage: nanomsg_reqrep.exe node0
// usage: nanomsg_reqrep.exe node1
int main( const int argc, const char **argv )
{
	if ( strncmp( NODE0, argv[ 1 ], strlen( NODE0 ) ) == 0 )
		return node0( "ipc:///tmp/reqrep.ipc" );
	else if ( strncmp( NODE1, argv[ 1 ], strlen( NODE1 ) ) == 0 )
		return node1( "ipc:///tmp/reqrep.ipc" );
	return 0;
}